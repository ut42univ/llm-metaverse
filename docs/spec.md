...
## 0.1 実装進捗サマリ (2025-09-25)

| ID    | 状態    | コメント                                                                        |
| ----- | ------- | ------------------------------------------------------------------------------- |
| FR-01 | 🔄 部分 | Colyseus 参加は自動開始。ニックネーム入力/制限 UI は未実装。                    |
| FR-02 | ⏳ 未着 | 参加フォーム未実装のため未達。                                                  |
| FR-03 | ✅ 達成 | 頭部位置を 20Hz 相当で送信、補間済み。                                          |
| FR-04 | ✅ 達成 | 頭部回転 (Euler) を送信・反映。                                                 |
| FR-05 | 🔄 部分 | XR モード・手ポーズ送信は実装。遠隔表示は簡易ボックス表示、アバター連携は未完。 |
| FR-06 | ✅ 達成 | カメラ姿勢を頭部として送信しリモートへ反映。                                    |
| FR-07 | ⏳ 未着 | ジェスチャー ID の送受信 UI/処理とも未実装。                                    |
| FR-08 | ⏳ 未着 | LiveKit 連携なし。                                                              |
| FR-09 | ⏳ 未着 | 距離減衰設定 UI/実装なし。                                                      |
| FR-10 | ⏳ 未着 | 入退室通知 UI 未実装。                                                          |
| FR-11 | ⏳ 未着 | 翻訳機能未実装。                                                                |
| FR-12 | ⏳ 未着 | 要約機能未実装。                                                                |
| FR-13 | ⏳ 未着 | 画像生成未実装。                                                                |
| FR-14 | ⏳ 未着 | 会話補助未実装。                                                                |
| FR-15 | ✅ 達成 | クライアント状態は非永続 (リロードで消去)。                                     |
| FR-16 | ⏳ 未着 | 障害時 UI/バックオフ未整備。                                                    |
| FR-17 | ⏳ 未着 | 接続インジケータ未実装。                                                        |

### NFR 指標の進捗

- NFR-P01 (同期レイテンシ): 実装上は補間を備えるが、正式な測定は未実施。
- NFR-P02 (初回ロード): 3D アセット軽量化は未測定。現状は開発機で ~5s 程度体感。
- NFR-P03 (帯域): 動きがない場合の送信抑制を実装済み。測定値は未取得。
- NFR-R01 (再接続): Colyseus 既定挙動のみで、UX 改善は未着手。
- NFR-U01/U02 (FPS・入力遅延): VR デバイスでの検証未実施。
- NFR-Sec01/Sec02: HTTPS/WSS 設定は未検証。現在はローカル開発証明書で起動。
...
5. AI 翻訳: 特定言語発話テキスト化(将来)を翻訳表示（MVP では手動入力テキスト翻訳でも可）。
...
- API/サービス: Colyseus (WebSocket), LiveKit (WebRTC), 生成 AI (HTTP/REST or gRPC)
...
| hands       | {left?:Pose,right?:Pose}       | コントローラ/ハンド         | Pose=位置+回転+状態 ID |
...
| 画像生成 | プロンプト                 | 画像 URL/Base64 | 25s          | 1 回     |
